...
import { MenuDataUpdate } from '@/services/MenuService';
...
export default function CourseForm({ initialCourseData, Message, SuccessMessage, Success }: menuFormProps) {
  const { register, handleSubmit, formState: { errors }, reset, setValue } = useForm({
    resolver: yupResolver(menuSchema),
    defaultValues: initialCourseData || {}
  });
  const [available, setAvailable] = useState<boolean>(initialCourseData?.available);
  const [hashtags, setHashtags] = useState<string[]>(initialCourseData?.hashtags);
  ...
  const onSubmit = async (data: any) => {
    try {
      const response = await MenuDataUpdate(initialCourseData._id, data, token);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="p-4">
      {InputForm.map((e) => (
        <PartInputs value={initialCourseData[e.IdName]} Type="form" key={e.id} IdName={e.IdName} LabelName={e.LabelName} type={e.type} Register={register} Errors={errors} />
      ))}
