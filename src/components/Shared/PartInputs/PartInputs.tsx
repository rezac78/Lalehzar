...
export default function PartInputs(props: InputProps) {
        ...
        return (
                <div>
                        {props.Type === 'search' ? null : <label htmlFor={uniqueId} className={props.Type === "auth" ? "block text-right text-titleColor text-sm font-bold mb-2" : "block text-sm font-medium text-gray-700"}>{props.LabelName}</label>}
