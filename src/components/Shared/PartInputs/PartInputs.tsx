...
export default function PartInputs(props: InputProps) {
        ...
        return (
                <div>
                        {props.Type === 'search' ? null : <label htmlFor={uniqueId} className={props.Type === "auth" ? "block text-right text-titleColor text-sm font-bold my-2" : "block text-sm font-medium text-gray-700 my-2"}>{props.LabelName}</label>}
                        {props.Type === 'search' ? <input onChange={(e) => props.setSearchTerm(e.target.value)} value={props.value} type={props.type} className={props.className} placeholder={props.placeholder} />
